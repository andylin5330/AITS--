    ...
    <el-card class="box-card" shadow="never" body-style="padding: 0; display: flex; flex-direction: column; height: 100%;">
      ...
      <div class="top-bar">
        <div class="status-indicator">
          ...
          <span class="title">API自动化测试用例智能生成</span>
        </div>
        <div class="actions">
          <el-button size="small" :icon="DocumentChecked" type="primary" plain @click="openSaveDialog" :disabled="messages.length === 0 || agentThinking">固化为正式用例</el-button>
        ...
        <div class="chat-area">
          <div class="chat-history" ref="chatHistoryRef">
            ...
            <div class="system-message">
              ...
              <h2 class="welcome-title">欢迎使用API场景测试用例生成智能体</h2>
              <p class="welcome-desc">请描述您的业务接口场景，例如：用户注册、登录、搜索商品、加入购物车的完整流程。我将帮您生成带有上下文依赖的自动化测试用例大纲。</p>
            </div>
  ...
  >
    <el-form :model="saveForm" :rules="saveRules" ref="saveFormRef" label-width="100px">
      <el-form-item label="所属项目" prop="project_id">
        <el-select v-model="saveForm.project_id" placeholder="请选择归属模块/项目" style="width: 100%">
           <el-option v-for="proj in projects" :key="proj.id" :label="proj.name" :value="proj.id" />
        </el-select>
      </el-form-item>
      <el-form-item label="用例名称" prop="name">
        <el-input v-model="saveForm.name" placeholder="请输入生成的场景名称" />
      </el-form-item>
    </el-form>
    <template #footer>
      <span class="dialog-footer">
        <el-button @click="saveDialogVisible = false">取消</el-button>
        <el-button type="primary" :loading="saving" @click="submitSave">确定入库</el-button>
      </span>
    </template>
  </el-dialog>
</template>
...
import { Service, Top, Clock, Timer, Delete, Download, User, DocumentChecked } from '@element-plus/icons-vue'
import { ElMessage } from 'element-plus'
import type { FormInstance, FormRules } from 'element-plus'
import axios from 'axios'
import MarkdownIt from 'markdown-it'
import { saveAgentCases } from '../../utils/api_ai_core'
...
const taskId = ref('api_agent_' + Math.random().toString(36).substr(2, 6))
  ...
  ws.onmessage = (event) => {
    try {
        ...
        if (data.event === 'status') {
            ...
            ElMessage.success('接口场景测试用例生成完成！')
            scrollToBottom()
        }
    } catch(e) { }
  }
...
const openSaveDialog = () => {
    saveForm.name = promptText.value ? promptText.value + ' - 测试场景' : 'AI生成的接口测试场景'
    saveForm.project_id = projects.value.length > 0 ? projects.value[0].id : undefined
...
const submitSave = async () => {
    ...
    await saveFormRef.value.validate(async (valid) => {
        if (valid) {
            saving.value = true
            try {
                // Get the entire agent generated text by mapping messages
                const mdContent = messages.value.filter(m => m.role === 'agent').map(m => m.content).join('\n\n')
                await saveAgentCases({
                    project_id: saveForm.project_id!,
                    name: saveForm.name,
                    content: mdContent
                })
                ElMessage.success('成功固化到接口测试用例库！')
                saveDialogVisible.value = false
            } catch(e) {
                ElMessage.error('固化用例失败！')
            } finally {
                saving.value = false
            }
        }
